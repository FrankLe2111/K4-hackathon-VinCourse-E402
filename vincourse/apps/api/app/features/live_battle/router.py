...
def parse_team_answer(raw_answer: str) -> tuple[str, str]:
    try:
    ...
    if len(reasoning) < 20:
        raise HTTPException(status_code=422, detail="Reasoning must contain at least 20 characters.")
...
def submit(request: GameSubmitRequest) -> GameResult:
    ...
    option_id, reasoning = parse_team_answer(request.answer)
    correct = option_id == "B"
    strong_reasoning = reasoning_explains_scaling(reasoning)
    ...
    ROOM_STATE["submitted_count"] = min(ROOM_STATE["joined_count"], ROOM_STATE["submitted_count"] + 1)
